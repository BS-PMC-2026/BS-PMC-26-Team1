...
class Question(db.Model):
    ...
    language = db.Column(db.String(32))  # e.g. python, javascript (code questions)
    execution_environment = db.Column(db.String(120))  # e.g. secure sandbox label
    compiler_info = db.Column(db.String(200))  # interpreter/compiler version label
    is_active = db.Column(db.Boolean, default=True)
...
class TheoryContent(db.Model):
    __tablename__ = "theory_content"
    id = db.Column(db.Integer, primary_key=True)
    module_id = db.Column(db.Integer, db.ForeignKey("modules.id"), nullable=False, unique=True)
    ...
    module = db.relationship("Module", backref=db.backref("theory_content", uselist=False))
    editor = db.relationship("User", backref=db.backref("updated_theory_content", lazy=True))
...
class LearningState(db.Model):
    ...
    last_route = db.Column(db.String(120), default="/learning-path")
    exercise_question_order_json = db.Column(db.Text, default="[]")
    exercise_seen_ids_json = db.Column(db.Text, default="[]")
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)

    user = db.relationship("User", backref=db.backref("learning_state", uselist=False))
    current_module = db.relationship("Module", backref=db.backref("active_learning_states", lazy=True))
