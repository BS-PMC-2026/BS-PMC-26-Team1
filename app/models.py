# Shared models — canonical source is 2_yosi/app/models.py
# This copy exists so 4_saaed can run standalone.
# Keep both files in sync when making model changes.
...
class LecturerProfile(db.Model):
    ...
    user = db.relationship(
        'User', backref=db.backref('lecturer_profile', uselist=False)
    )
...
class ExerciseAttempt(db.Model):
    ...
    selected_option_id = db.Column(
        db.Integer, db.ForeignKey('question_options.id'), nullable=True
    )
    ...
    user = db.relationship(
        'User', backref=db.backref('exercise_attempts', lazy=True)
    )
