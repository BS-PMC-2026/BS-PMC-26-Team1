...
def progress():
    ...
    attempts = (
        ExerciseAttempt.query.filter_by(user_id=user_id)
    ...
    eligibility = evaluate_certificate(user) if user else None
    db.session.commit()
    ...
    return render_template(
        ...
        stats=stats,
        demo_mode=False,
        certificate_eligible=bool(eligibility and eligibility.eligible),
    )
